...
<Accordion title="Optional: enable the public scoreboard">
  The frontend also serves a public stats site for your community. It already
  has its own URL: the frontend service's **second** domain, the one targeting
  port **81** (Settings, Networking). It shows data once you configure the
  scoreboard in the CRCON UI. Playing Hell Let Loose: Vietnam? Also change
  `HLL_GAME` to `hllv` on the backend service.
</Accordion>
